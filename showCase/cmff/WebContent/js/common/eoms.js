...
eoms.util = {
	...
	appendPage_ext411 : function(id, url, isUpdate, fn, scope) {
		try{
			if(Ext){
				var el = Ext.get(id);
				//alert(typeof el);
				 ...
				 * 
				 Ext.create('Ext.Panel', {
					...
					autoScroll : true, 
					renderTo : el,
					html:' <iframe scrolling="auto" frameborder="0" width="100%" height=500 src="'+url+'"> </iframe>'
				});
				 */
				/**
				 * 
	...
	appendPage_ext321 : function(id, url, isUpdate, fn, scope) {
		try{
			if(Ext){
				var el = Ext.get(id);
