...
//树的返回值反写
function toDoWritePartyWindowTree(win_id,inputArray, rtObj) {

	var textValue = inputArray[0];
	var textName = inputArray[1];
	var textCom = inputArray[2];
	...
	if(inputArray != null && rtObj != null && rtObj.length > 0){
		...
		for(var i=0; i<rtObj.length-1; i++) {
			allTextValue += rtObj[i]['returnValue'] + ",";
			allTextName += rtObj[i]['childName'] + ",";
			allTextCom += rtObj[i]['busContent'] + ",";
		}
		allTextValue += rtObj[rtObj.length-1]['returnValue'];
		allTextName += rtObj[rtObj.length-1]['childName'];
		allTextCom += rtObj[rtObj.length-1]['busContent'];
		if(textValue != null && textValue != ""){
			eval(textValue).value = allTextValue;
		}
		if(textName != null && textName != ""){
			eval(textName).value = allTextName;
		}
		if(textCom != null && textCom != ""){
			eval(textCom).value = allTextCom;
		}
	}
}
